...
# Caspi 2020. The version below assumes that the forward ('a') matrix
# and regularization matrices are provided as scipy.sparse.linalg
# LinearOperator objects, and it itself behaves as one as well.
class nlmap_operator(LinearOperator):
	def __init__(self, amat, regmat, drvmat, wgtmat, reg_drvmat, dtype=np.float32, reg_fac=1):
		self.amat = sparse_wrapper(amat)
		self.regmat = sparse_wrapper(regmat)
		self.drvmat = drvmat
		self.wgtmat = wgtmat
	...
	def _matvec(self,vec):
		chi2term = self.drvmat*(self.amat.rmatvec(self.wgtmat*(self.amat.matvec(self.drvmat*vec))))
		regterm = self.reg_drvmat*(self.reg_fac*self.regmat.matvec(self.reg_drvmat*vec))
		return (chi2term+regterm).astype(self.dtype_internal)

	def _adjoint(self):
		return self

# This version assumes that the 'a' matrix and regularization matrix
...
# scipy.sparse matrices are not scipy.sparse.linalg LinearOperators,
# and attempting to treat them as such will cause an error. This wrapper
# fixes that. It will work on any other object that has transpose and dot methods:
def sparse_wrapper(operator):
	if(linop_check(operator)):
		return operator
	else:
		return sparse_wrapper_class(operator)

# Check to see if operator is a scipy.sparse.linalg LinearOperator.
# Returns false for many scipy.sparse matrix types, even though they're
# linear operators, mathematically speaking.
def linop_check(operator):
	matvec = getattr(operator, 'matvec', None)
	rmatvec = getattr(operator, 'rmatvec', None)
	return callable(matvec) and callable(rmatvec)
	
class sparse_wrapper_class(LinearOperator):
	def __init__(self, operator):
		self.operator = operator
		self.shape = operator.shape
		self.dtype = operator.dtype
	def _matvec(self,vec):
		return self.operator.dot(vec)
	def _rmatvec(self,vec):
		return self.operator.transpose().dot(vec)
...
def sparse_nlmap_solver(data0, errors0, amat0, guess=None, reg_fac=1, func=None, dfunc=None, ifunc=None, regmat=None, silent=False,
	...
	alo = sparse_wrapper(amat0)
	guess0 = alo.rmatvec(np.clip(flatdat,np.min(flaterrs),None))
	guess0dat = alo.matvec(guess0)
	...
	guess = ((1+np.zeros(nsrc))*np.mean(flatdat)/np.mean(alo.matvec(1+np.zeros(nsrc)))).astype(dtype)
	if(flatguess): guess = ((1+np.zeros(nsrc))*np.mean(flatdat)/np.mean(alo.matvec(1+np.zeros(nsrc)))).astype(dtype)
	...
	if(adapt_lam):
		rlo = sparse_wrapper(regmat)
		reglam = (np.dot(dregfunc(svec)*(rlo.matvec(regfunc(svec))), dfunc(svec)*(amat0.T*(one/flaterrs)))/
				  np.dot(dregfunc(svec)*(rlo.matvec(regfunc(svec))), dregfunc(svec)*(rlo.matvec(regfunc(svec))))).astype(dtype)
	else: reglam = one
	...
	for i in range(0,niter):
		...
		if(lo_check):
			bvec = dguess*amat0.rmatvec(diags(weights)*(flatdat-amat0.matvec(func(svec)-svec*dfunc(svec))))
			bvec -= dregguess*(reg_fac*regmat.matvec(regfunc(svec)-svec*dregfunc(svec)))
		else:
